...
この一枚は、**ビルド指示書**である。読み手は人間ではなく、これを渡された Claude（主に Claude Code）を想定する。最終目的は、ここに定義した情報管理体系を **再利用可能な Claude Code プラグイン**（Skills と Hooks とスクリプトとテンプレートの束）として実装させることにある。プラグインを一度作れば、どのリポジトリでも同じ統治を効かせられる。これが「一枚を渡せばどこでも使い回せる」の意味である。

読み順は次のとおりとする。第1〜4章で「何を・なぜ作るか」を確定する。第5章で二つの調査文書のマージ判断（正準仕様の根拠）を示す。第6章が **正準リファレンス**（Skills と Hooks が共通で実装する単一の真実）である。第7〜10章が実装仕様（Skills／Hooks／Scripts／パッケージング）である。第11章がビルド順序の具体的指示である。第12〜14章が検証・破綻予測・チェックリストである。付録にテンプレートと出典を置く。
...
- 文書は資産ではなく**負債**である。各文書は存在を正当化しなければならない。生成はほぼ無料になったが、読む・保守する・信頼するコストは人間に残る。したがって既定の失敗様式は「散乱（sprawl）」であり、対策は「少なく書き、積極的に消し、各事実の正本を一つだけ保つ」ことである。
- LLM の典型的失敗（廃案復活・未決の既決化・仕様と実装の取り違え・古い前提の採用）は、いずれも「情報の地位（current か、廃案か、未決か、根拠か）が文書構造に書かれていない」ことに起因する。ゆえに設計の主眼は、量を捌くことではなく、各情報片に**地位・出所・依存・現行性**を付与して追跡可能にすることに置く。
- コンテキストは「多いほど良い」ではない。リポジトリのコンテキストファイル（CLAUDE.md / AGENTS.md）を肥大させると、コーディングエージェントの成功率はむしろ下がり、推論コストは増える（ETH Zürich & LogicStar.ai, arXiv:2602.11988, 2026）。ゆえに常時投入は「最小の、エージェントが他から知り得ない非自明な事実」に絞る。CLAUDE.md は知識の集積ではなく**薄いルーター**にする。
...
- **柱1（役割分担）**: 本体系の核心は「構造が予防・検出できること」と「人間とテストに委ねること」の分界にある。これを Claude Code の機構へ写像する。**Hook = 決定論的な強制（リンタ/CI に相当）**、**Skill = 手続き的知識と判断の補助**、**Script = Hook が呼ぶ検証器**、**テンプレート = 文書型の契約**、**CLAUDE.md/AGENTS.md = 常時投入の薄いルーター**。決定論で守れるものは Hook が守り、判断が要るものは Skill が支援する。
- **柱2（単一の正準仕様）**: Skills と Hooks は、別々の規則を持ってはならない。第6章の正準リファレンス（文書型・ステータス統制語彙・メタデータ・フォルダ・LLMコンテキスト区分・ライフサイクル・文章規範）を、両者が同一の真実として参照する。リンタの判定規則と、Skill が文書を作る規則は、同じ表から導かれる。
- **柱3（最小から始め、痛みが出たら育てる）**: 体系の重さは成熟度に段階化する。`docs-system-init` の既定は **Level 1 の最小5ファイル + 薄いルーター**であり、空のフォルダ群（カテドラル）を先に作らない。フォルダや重いメタデータは、その文書型が初めて必要になった時点で `doc-author` が遅延生成する。これは「空の足場自体が、避けたかった散乱である」という規律に従う。

### 1.3 保証できる範囲と、できない範囲（責任分界点）

**予防・検出・早期発見できるもの（構造と Hook と Script で対処）**
...
「100%の予防」は構造上不可能である。本プラグインの効能は、特定の失敗類型を**適切に運用された場合に検出・早期発見できる**ことに限定される。

---

## 2. 「ビルドが成功した」の定義（受け入れ基準）

ビルドの成否は、抽象的な「管理できている感」ではなく、**7つの観点それぞれについて、それを担保する Skill・Hook・Script・テンプレートを指させるか**で判定する。指させれば達成、指させなければ未達とする。

| # | 観点 | 達成条件（何を指させればよいか） |
|---|---|---|
| 1 | 探索性 | フォルダ・命名規則・Overview 索引で目的の文書に到達できる。`doc-author` が型コード命名を強制し、`docs-audit` が孤立文書を検出する。 |
| 2 | 現行性 | 各文書の `status` で現行が判別できる。リンタが統制語彙を強制し、監査が `review_by` 超過の current 文書を一覧化する。 |
| 3 | 追跡性 | Requirement→Spec→実装→Test→ADR を `depends_on`/`impacts` でたどれる。`change-impact` がトレーサビリティを生成・点検する。 |
| 4 | 変更耐性 | 変更時に更新すべき文書集合を依存リンクから列挙できる。`change-impact` の14ステップと、リンタの dead-link 検出。 |
| 5 | LLM耐性 | 常時投入が現行・禁止・非目標・未決に限定され、廃案本文は渡らない。SessionStart 注入と `llm-context-pack` と PreToolUse アーカイブガード。 |
...
加えて、次のメタ条件を満たすこと。**(a)** プラグインが `/plugin install` で他リポジトリへ配布できる。**(b)** `docs-system-init` が新規・既存いずれのリポジトリでも最小セットを安全に敷設できる（既存ファイルを破壊しない）。**(c)** すべての Script が標準ライブラリのみで動く（外部 pip 依存ゼロ。どこでも動くため）。**(d)** Hook の既定構成がエージェントを体感的に遅くしない（フルリポジトリ監査は毎ターンではなくコマンド/CI で実行する）。
...
- **B. 役割分担（決定論 vs 判断）**
  ...
  - B3 両者が共有する Script（検証器）の境界
- **C. Skills 実装**
  - C1 各 Skill のトリガー設計（過小発火を避ける description）
...
- **E. パッケージングと配布**
  ...
  - E3 リポジトリへの敷設（init Skill）とプレーン `.claude/` フォールバック
- **F. 検証**
  - F1 中心主張の妥当性（Toulmin）
...
- **Complication（複雑化）**: 生成物には地位がない。仕様・実装・調査・判断・廃案・未決が同じ粒度・同じ見た目で並ぶ。後から読むと、なぜそこにあるか、どれが現行か、何に依存するかが分からない。LLM 自身も古いログや廃案を現行として再採用する。一方で、対策として常時コンテキストを増やすと、エージェントの性能はむしろ下がる（arXiv:2602.11988）。手づくりの `.claude/` 設定は各人がばらばらに持ち、リポジトリ間で同期しない。
- **Question（問い）**: どのような Skills・Hooks・Script・テンプレートと、それを束ねる配布形態を設計すれば、人間と LLM の双方が地位・現行性・依存を追跡でき、変更と退行に耐え、かつ常時コンテキストを最小に保ったまま、**どのリポジトリでも同じ統治を再利用できるか**。
- **Answer（答え）**: 地位で分離する文書型体系と、出所・依存・現行性を必須メタデータ化する運用を、**Hook（決定論的強制）と Skill（手続き的支援）に役割分担**して実装する。両者は単一の正準仕様（第6章）を参照する。常時投入は薄いルーターと最小の確定事実に限定する。これらを単一プラグインに束ね、`/plugin install` で配布し、init Skill で各リポジトリへ最小敷設する。詳細を第5章以降で示す。

---

## 5. 統合方針（二文書のマージ判断）

二つのアップロード文書は、同じ主題の二版である。一方（`LLM開発情報管理体系.md`）は概念枠組みが網羅的で、24テンプレートと6観点の成熟度を持つ。他方（`compass_artifact`）は経験的により厳密で、7語の統制語彙・リッチなメタデータ・14ステップの変更フロー・ETH 研究の限界提示を持つ。正準仕様は、原則として後者の厳密さを採り、前者の網羅性で補う。判断と根拠を以下に固定する。
...
| フォルダ構成 | **11ディレクトリのハイフン版**（/00-overview … /99-archive + ルート AGENTS.md）。 | 決定・実装・テスト・運用を分離。ハイフンはパスで安全。地位と型でトップレベルが一意。 |
| 文書型 | **統合した単一表**（第6.1節）。地位の列を持つ分類表を骨格に、機能群（常時/Semantic/Episodic/運用変更/アーカイブ）で整理。 | 両文書の型を漏れなく一意化し、リンタと `doc-author` が同じ表を参照できる。 |
...
| 常時投入の重さ | **最小化**。CLAUDE.md/AGENTS.md は薄いルーター。常時投入は確定事実・非目標・廃案事実・未決・用語・Watchlist 要点に限定。 | arXiv:2602.11988 が示すとおり、肥大は性能低下とコスト増を招く。 |
...
本章は Skills と Hooks が共通で実装する**単一の真実**である。実装はすべてこの章の表に従う。仕様の変更は本章でのみ行い、Skills・Hooks のコードに規則を二重定義しない。

### 6.1 情報分類と文書型（単一表）

各文書は「型」と「地位」を持つ。型コードは ID の接頭辞であり、フォルダと既定の `llm_context` 区分を一意に決める。`doc-author` はこの表に従って文書を作り、リンタは `type` とフォルダと `status` の整合をこの表で検証する。
...
| REQ | Requirement（要求） | /10-product | current | task | 要求文（EARS 推奨）、優先度、受入基準参照、出所 | 実現方法（Spec/Arch へ） |
...
**llm_context の運用要点**: `always` = 薄いルーター（CLAUDE.md/AGENTS.md）+ OVERVIEW/GLOSSARY/NONGOAL/DECIDED/OPENQ/DEPREC（事実のみ）/WATCH（要点のみ）。`task` = そのタスクに直接関わる current の SPEC/ARCH/ADR/IMPL/TEST 等。`never` = RESEARCH/COMPET/LEGAL/INCIDENT/ARCHIVE の本文、古い会話ログ、機密。`always` 群でも本文全量ではなく**事実・要点のみ**を注入する（肥大回避）。

### 6.2 ステータス統制語彙

`status` は以下の7語のみを使う（ADR の `accepted` を含めて型別許可リストで検証）。これ以外を使わない。自由語を増やすと探索性と機械検証が崩れる。
...
| `deprecated` | 廃案。撤回済み。現行ではない。 | DEPREC, 撤回された各型 |
...
**形骸化を防ぐ設計**: `status` と `updated` は LLM に更新を代行させ、人間は承認のみ。`review_by` 超過の current 文書を `docs-audit` が「現行性疑い」として一覧化する。

### 6.4 フォルダ構成と命名規則

```
<repo>/
├── AGENTS.md                 # ルート直下。薄いルーター。最初に読む規約
...
**重要**: `docs-system-init` の既定はこの全ツリーを作らない。Level 1 の最小（`90-llm/decided-facts.md`, `90-llm/open-questions.md`, `10-product/non-goals.md`, `00-overview/glossary.md`, `00-overview/overview.md` の5ファイル + ルートの薄い AGENTS.md/CLAUDE.md）のみを敷設する。各フォルダは、その型の文書が初めて作られる時に `doc-author` が生成する。空の足場を先に作らない。
...
- 禁止: 日本語ファイル名の濫用、空白、版番号のファイル名埋め込み（version はメタデータか git で管理）、`design.md`/`memo.md` 等の文脈を伝えない抽象名。

### 6.5 LLM コンテキスト管理（3区分）

LLM に渡す情報を3区分し、廃案・未決・禁止を分離する。`llm-context-pack` Skill と SessionStart Hook がこれを実装する。

- **常時投入（llm_context: always）**: AGENTS.md（薄いルーター）、Decided Facts、Non-goals、Deprecated Item（撤回事実のみ）、Glossary、Regression Watchlist（要点のみ）。本文全量でなく事実・要点に絞る。
...
- **古い情報を証跡として渡す場合**: 「これは退役した旧仕様であり現行ではない」と地位を明示して渡す。地位ラベルなしに渡すと現行と誤読される。
...
12. 廃案・旧版・撤回済みを整理（Deprecated Item と Regression Watchlist に追記、Archive へ移動）。
...
- 復旧手順: 退行を検出 → Watchlist の根拠 ADR を確認 → 現行へ戻す Change Request を起票 → 原因分類（地位未記載か、テスト欠如か） → 監視点を強化 → Incident に記録。
...
- アーカイブ: 現行ではないが証跡価値がある（退役仕様、撤回案、旧価格）。`/99-archive` へ。
- 上位索引（Overview/Decided Facts）に載せる: 現行の判断に常時必要、または LLM に毎回読ませる必要があるもの。
- 廃案の再評価: 撤回案が別市場・別フェーズで有効になりうるものは、Deprecated Item に「再評価条件」を残し、Watchlist（戻してはならない退行）とは区別する。
...
- LLM っぽい空句の禁止（「重要なのは」「正面から」「不可欠」「掘り下げる」「多角的」「〜において」「非常に」等）。
...
Skill は「on-demand の専門知識」である。description が常時コンテキストに載り、本文はトリガー時のみ読み込まれる。**description がトリガーそのもの**であるため、三人称・主要ユースケース先頭・ユーザーが実際に言う語句の列挙で書く。Claude は単純な一手で済むタスクには Skill を使わない傾向（過小発火）があるため、description は少し「押しが強い」書き方にする。
...
### 7.1 `docs-system-init`（敷設）｜Level 0→2

- **description 要旨**: 「リポジトリに LLM 向け情報統治を導入する。`/docs` 構造の最小セットと、薄い AGENTS.md/CLAUDE.md、ステータス統制語彙、メタデータ規約、リンタ・監査スクリプト、Hook 設定を敷設する。新規リポジトリの初期化、既存リポジトリへの体系導入、`docs` の標準化、AGENTS.md の整備、を求められたら必ず使う。」
- **手続き**:
  1. 既存の `docs/` と AGENTS.md/CLAUDE.md を検出し、**破壊しない**（あれば差分提案、なければ新規）。
  2. 既定（Level 1）で最小5ファイル + 薄いルーターのみ敷設する（第6.4節）。フル11ツリーは作らない。
  ...
  5. 敷設後、第14章の「採用チェックリスト」を出力し、次のレベルへの移行条件を示す。
- **参照ファイル**: `references/levels.md`（Level 0–5 の状態・移行条件・失敗しやすい点）、`references/folder-layout.md`（第6.4節の詳細）。
- **スクリプト**: `scripts/scaffold.py`（最小敷設。既存ファイル非破壊）。
- **対応 Hook**: 敷設後は SessionStart 注入、PreToolUse ガード、PostToolUse リンタが効く。

### 7.2 `doc-author`（型付き文書の作成・更新）｜Level 1→2

- **description 要旨**: 「型付きの文書（仕様・要求・ADR・調査・Decided Facts・Non-goal など）を、正しいフロントマターと配置で作成・更新する。仕様を書く、ADR を起こす、Non-goal を足す、調査メモを残す、用語を定義する、決定済み事実を追加する、といった依頼で必ず使う。型の取り違え（調査に決定を書く等）と、廃案の現行ファイルへの放置を防ぐ。」
- **手続き**:
  ...
  4. 真価変化の昇格基準（第6.6節）に該当する場合は、元文書を残しつつ昇格先（ADR 等）を作り、`sources`/`supersedes` でリンクする。
...
- **対応 Hook**: PostToolUse リンタ（必須の相棒）、PreToolUse ADR 改変ガード。

### 7.3 `doc-review`（文章規範＋地位レビュー）｜Level 2→3

- **description 要旨**: 「文書または LLM 生成物に、日本語技術文書規範（k16shikano ベース）と地位レビューを適用する。文章をレビューする、LLM の出力を点検する、抽象的すぎる記述を直す、推量と断定の混在を直す、未決が既決として書かれていないか確認する、といった依頼で必ず使う。」
- **手続き**:
  1. 第6.7節のレビュー観点6項目を順に適用し、違反箇所を列挙する。
  2. 地位レビュー: `status` が内容と一致するか、未決（open）が断定で書かれていないか、current と draft が混ざっていないかを点検する。
...
- **description 要旨**: 「退行（撤回した方針や廃案の復活、決定の取り違え）を防ぐ。Decided Facts・Deprecated Item・Regression Watchlist を整備・点検し、提案中の変更や新規仕様が過去の決定と矛盾しないか確認する。『この方針は前に却下したはず』『廃案が復活していないか』といった文脈、および新仕様の提案時に必ず使う。」
- **手続き**:
...
- **手続き**:
  1. 第6.5節の3区分に従い、`always` 群（事実・要点のみ）+ タスク対象の current Spec + 関連 ADR を集約する。
  2. `never` 群（廃案本文・古いログ・未整理調査・機密）を明示的に除外する。証跡として古い仕様を含める場合は「退役・現行ではない」と地位を明示する。
  3. ETH 知見に従い**最小**に保つ。冗長な構造説明やリンタが担う規約は入れない（コスト増・性能劣化を招く）。
  4. 付録Bの Context Pack テンプレートに出力し、`/90-llm/` に CTXPACK として保存する。
- **参照ファイル**: `references/context-engineering.md`（3区分の運用、progressive disclosure、ETH 知見の要約と「最小限の非自明情報に絞る」原則）。
...
- **過小発火対策**: description は「〜の依頼で必ず使う」を含め、ユーザーが実際に使う語句を列挙する。
- **粒度**: 上記6つに限定し、安易に増やさない（型を増やしすぎる失敗の Skill 版を避ける）。「依存管理」は `change-impact` 内、「昇格」は `doc-author` 内に references として収め、独立 Skill にしない。
...
- **イベント / マッチャ**: `SessionStart`（matcher: `startup|resume|clear` 等）。
- **目的**: セッション開始時に、常時投入すべき「契約」を**最小**で注入する。注入内容は Decided Facts（current）+ Non-goals + Deprecated Item の**事実のみ** + Glossary の見出し + Regression Watchlist の「戻ってはならない事項」列。本文全量は注入しない（ETH 知見：肥大は性能低下とコスト増）。
...
- **(4) Bash 安全ガード（PreToolUse, matcher: `Bash`, if: `Bash(rm *)` 等）**: `/99-archive` の削除や破壊的コマンドを拒否する。証跡の喪失を防ぐ。汎用安全策。
- **(5) フルリポジトリ監査（手動 / CI / 任意で Stop）**: `docs-audit.py` を `/audit` コマンドまたは CI で実行する。dead link の全件、`review_by` 超過の current 文書、N 日以上 draft 放置、RESEARCH に決定混入、Watchlist 未保守 を一覧化する。**毎ターンは実行しない**（エージェントを遅くするため）。重い検査は per-turn ではなくここに置く。
...
- **速度を最優先する**: per-turn の Hook（PostToolUse リンタ）は単一ファイルのみ検証する。全件走査・依存グラフ・stale 検出は `docs-audit.py`（コマンド/CI）に隔離する。不要に発火する Hook はエージェントを著しく遅くする。
...
| `docs-audit.py` | コマンド/CI | リポジトリルート | 全件 dead link・stale current・draft 放置・型規律違反の一覧 |
...
| `scaffold.py` | docs-system-init Skill | レベル指定 | 最小5ファイル+薄いルーターを非破壊で敷設 |
...
def main():
    ...
    if meta.get("llm_context") and meta["llm_context"] not in {"always","task","never"}:
        errs.append(f"llm_context は always|task|never のいずれか: {meta['llm_context']}")
    ...
    for rk in ("depends_on","impacts","supersedes","superseded_by"):
        ...
        for v in vals:
            if v and v not in ids:
                errs.append(f"{rk} の参照先 '{v}' が存在しません（dead link）")
    # 8) 型規律（軽い検査）
...
def main():
    ...
    emit("撤回済み・禁止事項（事実のみ）", "90-llm/*deprecated*.md")
...
- **`collect-context.py`**: `llm_context` メタデータでフィルタし、`always` 群（事実・要点）+ 指定タスクの `task` 文書を収集、`never` を除外して Context Pack 雛形を出力。
- **`scaffold.py`**: 既存 `docs/` と AGENTS.md/CLAUDE.md を確認し、無いものだけを作成（**非破壊**）。既定は Level 1 の最小5ファイル + 薄いルーターのみ。フル11ツリーは作らない。

---

## 10. パッケージングと配布（Plugin）

「どこでも再利用」を満たす最良の形態は **Claude Code プラグイン**である。プラグインは Skills・Hooks・Scripts・テンプレートを一つのバージョン付き単位に束ね、`/plugin install` で配布・更新・信頼できる。プラグインは新機能を導入するのではなく、既存の能力を**梱包**する。すべての構成要素は単独でも `.claude/` に設定できるが、束ねることでチーム・複数リポジトリ間で同期する手間が消える。
...
- **スコープと上書き順**: Skill の探索は プロジェクト `.claude/skills/`（起動ディレクトリから親をたどりリポジトリルートまで）と 個人 `~/.claude/skills/` と プラグイン。Hook の設置場所は `~/.claude/settings.json`（全プロジェクト）/ `.claude/settings.json`（プロジェクト・コミット可）/ `.claude/settings.local.json`（gitignore）/ プラグイン `hooks/hooks.json` / Skill・agent フロントマター。

### 10.3 CLAUDE.md / AGENTS.md は薄いルーターにする

常時読まれる規約ファイルは**知識の集積にしない**。エントリポイント（`/docs/00-overview/overview.md`、Decided Facts、Non-goals、Watchlist）への**ルーター**に徹する。タスク固有の深さはリンク先に置く。肥大は性能低下とコスト増を招く（arXiv:2602.11988）。ひな型は付録C。
...
1. **正準リファレンスをコード化する**: 第6章の表（TYPE_FOLDER / STATUS / REQUIRED / llm_context 既定）を `scripts/_frontmatter.py` と `docs-linter.py` のデータ構造として固定する。規則をここに一元化し、以降のコードに二重定義しない。
...
5. **テンプレートと薄いルーターを用意する**: 付録A・C を `templates/` に置く。
6. **プラグインとして梱包する**: 第10.1節の構成と `plugin.json`・`hooks.json`。
7. **自己検証する**: 空のテストリポジトリで `docs-system-init` → 最小5ファイルが敷設されることを確認。わざと壊した文書（status 不正・dead link・型/フォルダ不一致）を書き、PostToolUse リンタが `continueOnBlock` で指摘することを確認。`/99-archive` への編集が deny されることを確認。SessionStart で最小契約のみが注入されることを確認。
8. **README を書く**: 導入手順、Skill 一覧、Hook 一覧、`/audit` の使い方、成熟度の上げ方（付録の採用チェックリストへリンク）。

実装中に正準仕様へ疑問が出たら、コードを勝手に分岐させず第6章を更新し、両者を一致させる（柱2）。

---

## 12. Toulmin による中心主張の検証

**主張（Claim）**: 「このプラグイン（Skills + Hooks + Scripts）を導入すれば、人間と LLM の双方が、どのリポジトリでも仕様・実装・意思決定を地位付きで管理でき、変更と退行に耐えられる。」

この主張を絶対保証として扱わず、構成要素に分解する。

- **根拠（Grounds）**: 地位分離（Decided Facts / Deprecated Item / Open Questions）と必須メタデータ（status/sources/updated）と依存リンク（depends_on/impacts）と退行テストを、Hook（決定論的強制）と Skill（手続き的支援）に役割分担して実装すれば、廃案復活・未決の既決化・仕様実装乖離は検出できる、という設計上の対応関係。
- **論拠（Warrant）**: LLM の典型的失敗は「情報の地位が構造化されていない」ことに起因する。地位を構造化し、Hook で機械的に強制すれば、検出可能性が上がる。
- **裏付け（Backing）**: 記録管理（ISO 15489 の真正性・信頼性・完全性・利用可能性）、要求工学（ISO/IEC/IEEE 29148 の追跡性）、ADR 実務（追記型・superseded 管理）、Claude Code の Hook/Skill 機構（決定論的制御と on-demand 専門知識）、コンテキスト最小化の経験的知見（arXiv:2602.11988）。
- **限定（Qualifier）**: 「管理できる」は「予防」ではなく「**検出・早期発見**」に限る。テスト化できる退行はテストで、それ以外は人間レビューで。条件は「メタデータが更新され、Hook が有効で、運用が回ること」。
- **反証（Rebuttal）**: 運用が形骸化すれば（メタデータ未更新、Watchlist 未保守）、本体系は「管理できている感」だけを生む。CLAUDE.md/AGENTS.md を肥大させれば、LLM の性能はむしろ下がる（arXiv:2602.11988）。仕様自体の誤り・外部依存の変化・LLM の確率的逸脱は、本体系では予防できない。Hook が過剰に発火すればエージェントが著しく遅くなる。
...
ある出来事が既に起きたと想定して振り返ると、将来の結果の理由を正しく特定する能力が上がる（prospective hindsight）。本ビルドが破綻した未来を想定し、原因を先回りする。

| # | 破綻シナリオ | 原因 | 対策（設計に織り込み済み） |
|---|---|---|---|
| 1 | Skill が呼ばれない | description が弱く過小発火 | 三人称・「〜の依頼で必ず使う」・実使用語句の列挙（第7章）。導入後に発火率を点検し description を調整。 |
| 2 | エージェントが遅い | Hook が重く毎ターン全件走査 | per-turn は単一ファイル検証のみ。全件監査は `docs-audit`（コマンド/CI）に隔離（第8.3節）。 |
| 3 | リンタが過検出で邪魔 | 規則が厳しすぎ false-positive | 必須は7項目に絞る。`continueOnBlock` で修正余地を残す。型規律は警告レベル（第6.3, 9.3節）。 |
| 4 | CLAUDE.md 肥大で性能低下 | 常時投入を増やした | 薄いルーター厳守。SessionStart は事実・要点のみ注入（第8.1, 10.3節、arXiv:2602.11988）。 |
| 5 | メタデータ形骸化 | 必須が多すぎ手動更新が続かない | 必須を4＋3項目に限定。status/updated は LLM 代行＋人間承認。`review_by` 超過を監査が一覧化（第6.3節）。 |
| 6 | フォルダがカテドラル化 | 空ツリーを先に作った | init 既定は最小5ファイル。フォルダは初使用時に遅延生成（第6.4, 7.1節）。 |
| 7 | 廃案が現行として復活 | 廃案の地位が文書化されず | Deprecated Item + Watchlist + SessionStart 常時投入 + PreToolUse アーカイブガード（第6.6, 8.1節）。 |
...
- [ ] `docs-system-init` 既定が最小5ファイル＋薄いルーターのみを非破壊で敷設する。
- [ ] CLAUDE.md/AGENTS.md が薄いルーターである。
- [ ] プラグインとして梱包され `/plugin install` で配布できる。
...
- [ ] status 統制語彙7語を理解した。
- [ ] プラグインを有効化（または `.claude/` にフォールバック敷設）した。

### 14.3 運用チェックリスト

- [ ] 新規文書に id/title/type/status/owner/updated/sources を付けた。
- [ ] `review_by` を過ぎた current 文書を点検した（`docs-audit`）。
...
llm_context: always
---
# 撤回済み事項（Deprecated Item）
<!-- 撤回した事項の事実のみ。本文（廃案の中身）は書かない -->
- 撤回事項: <...> ｜ 撤回日: YYYY-MM-DD ｜ 後継: <現行のリンク or なし> ｜ 再評価条件: <別市場/別フェーズで有効になる条件 or なし>
```

---

## 付録C. 薄いルーターひな型（`AGENTS.md` / `CLAUDE.md`）

知識を書かない。エントリポイントへのリンクに徹する。肥大は性能低下を招く（arXiv:2602.11988）。
...
- Plugins（Skills/Hooks/MCP の梱包と配布、`hooks/hooks.json`、`/plugin install`）
...
**注記**: 本設計書の経験的数値（ETH 研究の成功率・コスト変化等）はアップロード文書と一次資料の記述に基づく。「100%保証」に類する表現は用いない。本体系の効能は、適切な運用下での特定失敗類型の検出・早期発見に限定される。
