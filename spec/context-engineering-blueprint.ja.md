...
| 6 | 認知負荷 | 一文書一目的、粒度が揃い、抽象語でぼかさない。`doc-review` のルーブリック（付録E）。 |
...
**運用版（論理チェックを含む全文）**: `doc-review` が用いる日本語・論理チェック規範の全文を付録Eに収録する（出典: JTCA『日本語スタイルガイド』／波頭亮『思考・論理・分析』）。
...
| 5 | メタデータ形骸化 | 必須が多すぎ手動更新が続かない | 必須を4＋3項目に限定。status/updated は LLM 代行＋人間承認。`review_by` 超過を監査が一覧化（第5.3節）。 |
...
## 付録E. 日本語・論理チェック規範（doc-review 運用版・全文）

`doc-review`（第6.3節）が用いる、日本語と論理のチェック規範を全文収録する。出典は、日本語＝JTCA『日本語スタイルガイド』（簡潔・明快・誤解されない）、論理＝波頭亮『思考・論理・分析』。「日本語」節のみを適用するチェックを `jp-style-check`、論理を含む全体を `logic-jp-check` と呼ぶ。本文・LLM 出力の双方に適用し、引っかかった点だけを出力する（問題なしは書かない＝肥大を避ける）。
