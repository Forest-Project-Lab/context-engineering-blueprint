...
1. 文書は資産ではなく**負債**である。各文書は存在を正当化しなければならない。生成はほぼ無料になったが、読む・保守する・信頼するコストは人間に残る。したがって既定の失敗様式は「散乱（sprawl）」であり、対策は「少なく書き、積極的に消し、各事実の正本を一つだけ保つ」ことである。
2. LLM の典型的失敗（廃案復活・未決の既決化・仕様と実装の取り違え・古い前提の採用）は、いずれも「情報の位置づけ（current か、廃案か、未決か、根拠か）が文書構造に書かれていない」ことに起因する。ゆえに設計の主眼は、量を捌くことではなく、各情報片に**位置づけ・出所・依存・現行性**を付与して追跡可能にすることに置く。
...
- **柱3（最小から始め、不便が出てから育てる）**: 体系の重さは成熟度に段階化する。`docs-system-init` の既定は **Level 1 の最小5ファイル + 最小限の案内**であり、空のフォルダを先に作らない。フォルダや重いメタデータは、その文書型が初めて必要になった時点で `doc-author` が遅延生成する。これは「空の足場自体が、避けたかった散乱である」という規律に従う。
...
二つのアップロード文書は、同じ主題の二版である。一方（`LLM開発情報管理体系.md`）は概念枠組みが網羅的で、24テンプレートと6観点の成熟度を持つ。他方（`compass_artifact`）は経験的により厳密で、7語の統制語彙・リッチなメタデータ・14ステップの変更フロー・ETH 研究の限界提示を持つ。正本仕様は、原則として後者の厳密さを採り、前者の網羅性で補う。判断と根拠を以下に固定する。
...
**llm_context の運用要点**: `always` = 最小限の案内（CLAUDE.md/AGENTS.md）+ OVERVIEW/GLOSSARY/NONGOAL/DECIDED/OPENQ/DEPREC（事実のみ）/WATCH（要点のみ）。`task` = そのタスクに直接関わる current の SPEC/ARCH/ADR/IMPL/TEST 等。`never` = RESEARCH/COMPET/LEGAL/INCIDENT/ARCHIVE の本文、古い会話ログ、機密。`always` 群でも本文全量ではなく**事実・要点のみ**を注入する（肥大回避）。
...
**重要**: `docs-system-init` の既定はこの全ツリーを作らない。Level 1 の最小（`90-llm/decided-facts.md`, `90-llm/open-questions.md`, `10-product/non-goals.md`, `00-overview/glossary.md`, `00-overview/overview.md` の5ファイル + ルートの最小限の AGENTS.md/CLAUDE.md）のみを配置する。各フォルダは、その型の文書が初めて作られるときに `doc-author` が生成する。空の足場を先に作らない。
...
| `docs-audit.py` | コマンド/CI | リポジトリルート | 全件 dead link・陳腐化した current・draft 放置・型規律違反の一覧 |
...
常時読まれる規約ファイルは**知識の集積にしない**。エントリポイント（`/docs/00-overview/overview.md`、Decided Facts、Non-goals、Watchlist）への**案内**に徹する。タスク固有の深さはリンク先に置く。肥大は避ける（前提3）。ひな型は付録C。
...
知識を書かない。エントリポイントへのリンクに徹する。肥大は避ける（前提3）。
