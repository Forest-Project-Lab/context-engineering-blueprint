...
- LLM の典型的失敗（廃案復活・未決の既決化・仕様と実装の取り違え・古い前提の採用）は、いずれも「情報の位置づけ（current か、廃案か、未決か、根拠か）が文書構造に書かれていない」ことに起因する。ゆえに設計の主眼は、量を捌くことではなく、各情報片に**位置づけ・出所・依存・現行性**を付与して追跡可能にすることに置く。
- コンテキストは「多いほど良い」ではない。リポジトリのコンテキストファイル（CLAUDE.md / AGENTS.md）を肥大させると、コーディングエージェントの成功率はむしろ下がり、推論コストは増える（ETH Zürich & LogicStar.ai, arXiv:2602.11988, 2026）。ゆえに常時投入は「最小の、エージェントが他から知り得ない非自明な事実」に絞る。CLAUDE.md は知識の集積ではなく、現行文書への入口だけを示す**薄い案内**にする。
...
**Claude Code 用の単一プラグイン `context-engineering-blueprint` を実装する。** このプラグインは、(a) 情報管理の手続きと判断を担う **6 個の Skill**、(b) 不変条件を機械的に強制する **少数の Hook**、(c) Hook が呼ぶ **決定論的バリデータ・スクリプト群**、(d) 文書型テンプレート、(e) 薄い案内としての CLAUDE.md / AGENTS.md ひな型、から成る。これを任意のリポジトリで有効化すれば、人間と LLM の双方が情報の位置づけ・現行性・依存を追跡でき、変更と退行に耐えられる開発を継続できる。

### 1.2 主張を支える3本の柱

- **柱1（役割分担）**: 本体系の核心は「構造が予防・検出できること」と「人間とテストに委ねること」の分界にある。これを Claude Code の機構へ写像する。**Hook = 決定論的な強制（リンタ/CI に相当）**、**Skill = 手続き的知識と判断の補助**、**Script = Hook が呼ぶ検証器**、**テンプレート = 文書型の契約**、**CLAUDE.md/AGENTS.md = 常時投入の薄い案内**。決定論で守れるものは Hook が守り、判断が要るものは Skill が支援する。
...
- **柱3（最小から始め、痛みが出たら育てる）**: 体系の重さは成熟度に段階化する。`docs-system-init` の既定は **Level 1 の最小5ファイル + 薄い案内**であり、空のフォルダを先に作らない。フォルダや重いメタデータは、その文書型が初めて必要になった時点で `doc-author` が遅延生成する。これは「空の足場自体が、避けたかった散乱である」という規律に従う。
...
- **Question（問い）**: どのような Skills・Hooks・Script・テンプレートと、それを束ねる配布形態を設計すれば、人間と LLM の双方が位置づけ・現行性・依存を追跡でき、変更と退行に耐え、かつ常時コンテキストを最小に保ったまま、**どのリポジトリでも同じ統治を再利用できるか**。
- **Answer（答え）**: 位置づけで分離する文書型体系と、出所・依存・現行性を必須メタデータ化する運用を、**Hook（決定論的強制）と Skill（手続き的支援）に役割分担**して実装する。両者は単一の正準仕様（第6章）を参照する。常時投入は薄い案内と最小の確定事実に限定する。これらを単一プラグインに束ね、`/plugin install` で配布し、init Skill で各リポジトリへ最小敷設する。詳細を第5章以降で示す。
...
| フォルダ構成 | **11ディレクトリのハイフン版**（/00-overview … /99-archive + ルート AGENTS.md）。 | 決定・実装・テスト・運用を分離。ハイフンはパスで安全。位置づけと型でトップレベルが一意。 |
...
| 常時投入の重さ | **最小化**。CLAUDE.md/AGENTS.md は薄い案内。常時投入は確定事実・非目標・廃案事実・未決・用語・Watchlist 要点に限定。 | arXiv:2602.11988 が示すとおり、肥大は性能低下とコスト増を招く。 |
...
**llm_context の運用要点**: `always` = 薄い案内（CLAUDE.md/AGENTS.md）+ OVERVIEW/GLOSSARY/NONGOAL/DECIDED/OPENQ/DEPREC（事実のみ）/WATCH（要点のみ）。`task` = そのタスクに直接関わる current の SPEC/ARCH/ADR/IMPL/TEST 等。`never` = RESEARCH/COMPET/LEGAL/INCIDENT/ARCHIVE の本文、古い会話ログ、機密。`always` 群でも本文全量ではなく**事実・要点のみ**を注入する（肥大回避）。
...
- **常時投入（llm_context: always）**: AGENTS.md（薄い案内）、Decided Facts、Non-goals、Deprecated Item（撤回事実のみ）、Glossary、Regression Watchlist（要点のみ）。本文全量でなく事実・要点に絞る。
...
- **手続き**:
  ...
  2. 既定（Level 1）で最小5ファイル + 薄い案内のみ敷設する（第6.4節）。フル11ツリーは作らない。
...
- **手続き**:
  ...
  4. 価値の変化の昇格基準（第6.6節）に該当する場合は、元文書を残しつつ昇格先（ADR 等）を作り、`sources`/`supersedes` でリンクする。
...
| `scaffold.py` | docs-system-init Skill | レベル指定 | 最小5ファイル+薄い案内を非破壊で敷設 |
...
- **`scaffold.py`**: 既存 `docs/` と AGENTS.md/CLAUDE.md を確認し、無いものだけを作成（**非破壊**）。既定は Level 1 の最小5ファイル + 薄い案内のみ。フル11ツリーは作らない。

---

## 10. パッケージングと配布（Plugin）

「どこでも再利用」を満たす最良の形態は **Claude Code プラグイン**である。プラグインは Skills・Hooks・Scripts・テンプレートを一つのバージョン付き単位に束ね、`/plugin install` で配布・更新・信頼できる。プラグインは新機能を導入するのではなく、既存の能力を**一つのパッケージにまとめる**。すべての構成要素は単独でも `.claude/` に設定できるが、束ねることでチーム・複数リポジトリ間で同期する手間が消える。
...
### 10.3 CLAUDE.md / AGENTS.md は薄い案内にする

常時読まれる規約ファイルは**知識の集積にしない**。エントリポイント（`/docs/00-overview/overview.md`、Decided Facts、Non-goals、Watchlist）への**案内**に徹する。タスク固有の深さはリンク先に置く。肥大は性能低下とコスト増を招く（arXiv:2602.11988）。ひな型は付録C。
...
5. **テンプレートと薄い案内を用意する**: 付録A・C を `templates/` に置く。
...
**主張（Claim）**: 「このプラグイン（Skills + Hooks + Scripts）を導入すれば、人間と LLM の双方が、どのリポジトリでも仕様・実装・意思決定を位置づけ付きで管理でき、変更と退行に耐えられる。」
...
| 4 | CLAUDE.md 肥大で性能低下 | 常時投入を増やした | 薄い案内厳守。SessionStart は事実・要点のみ注入（第8.1, 10.3節、arXiv:2602.11988）。 |
...
- [ ] `docs-system-init` 既定が最小5ファイル＋薄い案内のみを非破壊で敷設する。
- [ ] CLAUDE.md/AGENTS.md が薄い案内である。
...
## 付録C. 薄い案内ひな型（`AGENTS.md` / `CLAUDE.md`）
...
**注記**: 本設計書の経験的数値（ETH 研究の成功率・コスト変化等）はアップロード文書と一次資料の記述に基づく。「100%保証」に類する表現は用いない。本体系の効果は、適切な運用下での特定失敗類型の検出・早期発見に限定される。
