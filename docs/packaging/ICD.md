...
created: 2026-06-30
updated: 2026-06-30
sources: [spec/context-engineering-blueprint.ja.md §4.3]
...
- **plugin-packaging**: 配布は `/plugin install` で行う。プラグインを配置できないときは `.claude/` へ退避する。スクリプトは標準ライブラリだけで動く。`plugin.json` の最小キーは、name=`context-engineering-blueprint`、version=`0.1.0`、license=MIT の三つである。
