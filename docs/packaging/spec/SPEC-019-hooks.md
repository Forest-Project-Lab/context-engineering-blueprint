...
- 縮小構成 `hooks/hooks.level2.json` は、全構成から SessionEnd の `docs-audit.py` と PostToolUse の `policy-guard.py` を外したものである。監査と依存グラフは Level 3 以降に置く。起動後のブロックには依存グラフが要るからである `[R5]`。
