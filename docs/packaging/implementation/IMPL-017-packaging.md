...
- `hooks/hooks.json` の PostToolUse は二つのスクリプトを配列で並べ、その順で起動する。`policy-guard.py` を先に、`docs-linter.py` を後に書く `[R7]`。配列の並びが起動順を決める。
...
- PostToolUse の二つのスクリプトの順を取り違えると、起動後に見つけた境界違反がブロックされず、助言だけで終わる。この配列順は退行監視の対象とする（WATCH を参照）。
